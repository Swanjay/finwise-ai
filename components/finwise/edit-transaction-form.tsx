...
export function EditTransactionForm({
  ...
  const { updateTransaction, tags: savedTags, addTag: saveTag, wallets } = useFinwise()
  ...
  const [location, setLocation] = useState<LocationData | null>(transaction.location ?? null)
  const [walletId, setWalletId] = useState(transaction.walletId ?? wallets[0]?.id ?? 'cash')
    ...
    <form onSubmit={submit} className="flex flex-col gap-4">
      ...
      <Button type="submit" disabled={!valid} className="h-12">
        {isSubmitting ? (
          ...
          <>
            <Check className="size-5" /> Simpan Perubahan
          </>
        )}
      </Button>
    </form>
